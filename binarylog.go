...
func newFloat(n int64, fracBits, maxFracBits int) float {
	f := float{n: big.NewInt(n), fracBits: fracBits, maxFracBits: maxFracBits}
	f.normalize()
	return f
}
...
func BinaryLog(n int64, mantissaBits int) (characteristic int, mantissa *big.Int) {
	if n <= 0 || mantissaBits < 0 {
		panic("invalid argument of BinaryLog")
	}

	characteristic = Log2Uint64(uint64(n))
